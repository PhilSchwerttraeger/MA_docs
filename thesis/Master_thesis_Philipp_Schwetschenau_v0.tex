...
Being the central artifact in the process of model-based security engineering we want to describe a very basic security model -- the identity-based access control model - and a well-established and widely used extension of it -- the role-based access control model.
In the course of this work, we need to construct security policies and models to explain, execute and visualize the security engineering in context of the Entity-Labeling aspect.
Those models will be RBAC models.
...
With IBAC requests are managed according to the requester's identification \citep{Lampson74}.
A set of rules determines whether a request is permitted or forbidden.
These rules can be notated in form of a access control matrix or of access control function.
An access control function is defined as following:
...
\begin{itemize}
\item $S$ is a set of subject identifiers,
\item $O$ is a set of object identifiers, and
