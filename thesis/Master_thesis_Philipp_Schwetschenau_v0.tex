...
\begin{figure}[htb]
	\centering
	\includegraphics[scale=1]{../figures/chapter2/Sandhu_notation.png}
...
Figure \ref{fig:ERD_minimal_example} shows a minimal example of an ERD.
...
\begin{itemize}
\setlength\itemsep{0em}
\item A movie is connected to exactly one genre (left-to-right).
\item A genre is connected to one or more movies (right-to-left).
\end{itemize}
...
A primary key attribute is graphically indicated with an underlined name (see \textit{MovieID} in figure \ref{fig:ERD_example}).
...
As seen in figure \ref{fig:ERD_as_UML} both diagrams differ in their appearance in almost all aspects -- except the depiction of entites -- but still are easy to translate into each other.
