...
It models a system with active (subjects) and passive entities (objects).
...
This concept reduces redundancy in systems with large or fast growing sets of subjects and objects and also models real scenarios closer to reality (e.g. job positions or departments in a company or organization).
...
\item $UA \subseteq U \times R$ as \textit{user-role relation}
\item $PA \subseteq P \times R$ as \textit{permission-role relation}
\item $user: S \rightarrow U$ as assignment, that maps every session to a user
\item $roles: S \rightarrow 2^R$ as assignment, that maps every session to set of roles
\end{itemize}
\end{xdefinition}

\noindent
RBAC is a family of security models, because there is a base model (RBAC$_0$) and models that extend the base model.
...
RBAC$_2$ extends RBAC$_0$ with constraints to restrict values for certain model components.
...
\subsection{RBAC notation by \cite{Sandhu96}}
\label{RBAC_notation}
\cite{Sandhu96} used a graphical notation in his work to visualize the RBAC model with all its elements and relationships.

\begin{figure}[htb]
	\centering
	\includegraphics[scale=0.3]{../figures/chapter2/RBAC0.png}
	\caption{Graphical notation used by \cite{Sandhu96}}
...
\begin{figure}[htb]
	\centering
	\includegraphics[scale=0.37]{../figures/chapter2/ERD_example.png}
	\caption{ER diagram with multiplicities and primary keys}
...
As seen in the last example of figure \ref{fig:ERD_notations} ER can also be modeled in an UML-style notation.
