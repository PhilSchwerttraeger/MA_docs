...
Section \ref{security_models} introduces two fundamental security models: IBAC and RBAC.
It also describes a graphical notation for depicting RBAC models.
...
In IBAC requests are managed according to the requester's unique identification \citep{Lampson74}.
...
\label{RBAC}
With an increasing number of subjects and objects IBAC's ACFs/ACMs get very large and hard to manage effectively.
...
\cite{Amthor18} also used a graphical depiction (see figure \ref{fig:EL_diagram}) that seem to be loosely based on the notation by \cite{Sandhu96} (see section \ref{RBAC_notation}).
...
\label{HEL}
To further improve scalability another level of indirection can be added to a model which already has one level of indirection (e.g. adding a role hierarchy like RBAC has).
...
So definition \ref{definition:EL} defines EL just for the case where $m=1$.
...
UML and ER come with reasonable concepts and opportunities to illustrate structural information and semantic properties (for types of information visualization in science see section \ref{perception}).
...
Examples for elements are classes, components, notes, objects, packages, interfaces, ports, etc.~(see figure \ref{fig:UML_elements}).
Elements can be closely tied to specific diagram types (e.g.~components to component diagrams).

\begin{figure}[htb]
	\centering
	\includegraphics[scale=0.5]{../figures/chapter2/UML_elements.png}
...
The notation variants only differ in the depiction of their relationships.
Here is a short overview of how some of them differ in their appearance:
...
One is to present absolute or relative primitive values (tables, bar graphs, pie charts) and one to depict structural information and semantic properties (specialized graphical systems like the class diagram of UML (see section \ref{UML}), the Chen notation of ERD (see section \ref{ER}), electrical block diagram, etc.).
...
This results in phenomena like completing incomplete forms, favoring closed objects or treating groups of objects as units.
...
Homogeneously designed objects are associated as belonging together, although they may not be in close range to each other.
This effect can be induced by \textit{Gestalt factors} like shape, color, brightness, size, orientation, rotation, etc.
