...
With this said we want to start off with the graphical notation \cite{Amthor18} uses to visualize the EL aspect and try to expose and analyze its two major problems: ambiguously depicted relationships and inelegant categorization.

% Describe the 2 general problems
\begin{description}
\item[1) Ambiguously depicted relationships] \hfill \\
...
\item[2) Ambiguously depicted categorization] \hfill \\
...
The horizontal alignment seems similar to a table categorization with columns, but unlike a table the graphical depiction uses arrows to show relationships between elements.
...
But arrows do not always connect elements across one section.
It is possible that they connect elements that are far away from each other or belong to the same section (e.g.~\textit{RH} in figure \ref{fig:EL_diagram}).
So arrows that connect two elements from different section will automatically be associated with them, which is not desired in many cases.

Expanding this concept into two dimensions will maintain the same issues.


\end{description}


% Applying Optical Perception

% Applying Gestalt Laws
%This law allows classifying or logically grouping multiple spatially distributed objects.
