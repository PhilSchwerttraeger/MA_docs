...
So for reducing the visual complexity whenever needed, and to let the user have a choice how prominent certain parts should be depicted, there is need for a concept for depicting information in a compact way.
This is the secondary design goal of the graphical specification language.
Specific solutions towards this goal are mentioned in the following section whenever it is reasonable applicable.
...
\noindent
Other than problem 2, power sets are applied to just one set at once.
...
The rhombus shape in solution to problem 2 also allows expressing a power set by slotting it in ahead with an appropriate symbol inside (e.g.\ $\euscr{P}$).
Together with annotations from solution to problem 1 we now have two semantically equivalent methods of depicting power sets.
This gives the modeler the choice how to express power sets.
The decision may vary from element to element and come with individual benefits.
While using annotations is more space-saving and compact, the rhombus shape slotted in ahead can act as its own entity and multiple other relationships can refer to it (without using annotation duplicates for every single relationship).
Regarding automated model-generation from imported data, the decision of what depiction to use this could be based on the number of references a power set has: annotation for one reference, rhombus element for two or more references.
...
Similar to the first method this gives the modeler the option to adjust to the desired level of detail.
...
When elements are participating in at least one other points of the model the method of graphical depiction outside of the element should be preferred due to less visual overlapping.
\end{description}
...
Depicting EL categories by horizontal alignment is removed and replaced with coloring.
...
\noindent
Alternatively they can be depicted completely graphically with the help of other elements (introduced in the following section \ref{relationships}).
Figure \ref{fig:subsets_alternative} shows such variant.
It is semantically equivalent to figure \ref{fig:subsets}.
For their individual benefits see \textit{Solution to Problem 5: Visual Complexity} from the previous section.

\begin{figure}[htb]
	\centering
	\includegraphics[scale=0.4]{../figures/chapter3/subsets_alternative.png}
	\caption{Subset depiction variant}
	\label{fig:subsets_alternative}
\end{figure}

\noindent
The internal definition of sets like $P = 2^{O \times OP}$ in $RBAC_0$ (see definition \ref{definition:RBAC} on page \pageref{definition:RBAC}) can be modeled graphically, but in cases like this, where all sets in the formula are not referenced at any other point of the model, the formula would probably be the better choice.
But note that unlike the subset depiction above a formula can not depict the category membership(s) of participating sets and is therefore \textit{not} semantically equivalent to its graphical counterpart.
If participating sets have EL categories assigned and are therefore relevant for the EL model using the formula always induce a loss of information.
The graphical depiction is probably favored in such cases.
...
Rhombus shapes do not necessarily need a name or identifier.
Also they can be left uncolored.
...
On an abstract level the graph is structured in nodes and edges, just like an mathematical graph.
Based on this one might assume this being a bipartite graph, because edges connect sets with other elements.
But due to mappings being also depicted with the help of graphical shapes, but are no sets at all, this is not the case.
Therefore the graph is not bipartite.

The reason for this is that mappings must provide a surface to hold their color to be able to get categorized properly.
Therefore they have to be encapsulated in a geometrical shape just like the other elements, too.
A depiction variant would be to not position the shape and its concent in the middle of its arrow, but above or below it with at least one point of contact.
But due to lines and arrows are not limited to run straight horizontally or vertically, this might induce confusion in distinguishing multiple adjacent elements in large models with less space.
