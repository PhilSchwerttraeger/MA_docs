...
%Ablauf
In the first section the general idea of how to achieve this and the language concept are explained.
...
Due to being highly tailored to one specific domain established notations and visualization systems like UML or ER (see section \ref{graphical_notations}) are impractical for direct reuse in another domain.
Generally a new domain (like EL) will also require a newly designed graphical system, but in reality certain concepts and ideas from related systems are getting (partly) adopted and adapted accordingly, when they fit semantically.
With or without adoption: A newly designed graphical system should be designed in a way its components are not ambiguously to systems from similar or related domains to avoid misunderstanding or barriers in learning.
With this said we want to start off with the graphical notation \cite{Amthor18} uses to depict the EL aspect and try to expose its problems.

% Describe the 2 general problems
%\begin{description}
%\item[1) Safe Exploration]\hfill \\

% Details on problem 1
When comparing the visual summary (figure \ref{fig:EL_diagram}) with the table form (figure \ref{fig:EL_table}) we will find several inconsistencies.
...
There is no example mapping in the depicted example, but we can easily imagine a simple function with two arguments, e.g.~a mapping that takes subjects and objects identifications to decide whether an action is allowed or not in the IBAC model (see section \ref{IBAC})):

\begin{align*}
S \times O \rightarrow R 
\end{align*}

To draw an arrow that comes from $S$ and $O$, connects in one point and ends with an arrow head at $R$ is not sufficient in terms of consistency, because the operator can vary.


% Details on problem 2
