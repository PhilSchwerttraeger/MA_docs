...
\label{gsl_design}
In this chapter we will develop a system to graphically model the Entity Labeling aspect (section \ref{EL}) of Aspect-oriented Security Engineering.
In the first section \ref{gsl_concept} the general idea of how to achieve this and the language concept are explained.
For this the notation used by \citep{Amthor18} is investigated regarding its completeness, consistency and general weaknesses.
In the section \ref{gsl_elements} the different elements the system uses are explained, followed by section \ref{gsl_structure}, which depicts the overall structure of the system.
...
Generally a new specialized domain like EL will also require a newly designed graphical system, but in reality certain concepts and ideas from related systems are getting (partly) adopted and adapted accordingly, when they semantically fit.
...
With this said we want to start off with the graphical notation \cite{Amthor18} uses to depict the EL aspect and try to expose and analyze its two major problems: ambiguously depicted relationships and inelegant categorization.
...
One is that all mappings ignore the presence of power sets, e.g.~$ua: U \rightarrow 2^R$ is drawn as a simple arrow from $U$ to $R$.
The inconsistency becomes clear when converting the graphical statement back to a mathematical one:
\begin{align*}
(U \rightarrow 2^R) \not\equiv (U \rightarrow R)
\end{align*}

Also mappings can have multiple input arguments.
There is no example mapping in the depicted example, but we can easily imagine a simple mapping with two arguments, e.g.~a mapping that takes subjects and objects identifications to decide whether an action is allowed or not (IBAC, see section \ref{IBAC}):
\begin{align*}
S \times O \rightarrow R 
\end{align*}

Drawing an arrow from $S$ and $O$, merging in one point and ending with an arrow head in $R$ is not sufficient in terms of consistency, because the operator (here: $\times$) can vary (may be $\cup$ or $\cap$ just as well).
Beyond that that, when handling three or more arguments the order of operations (may be indicated with brackets) has to be considered and depicted as well.

\item[2) Ambiguously depicted categorization] \hfill \\
The EL categories are depicted by
\end{description}
