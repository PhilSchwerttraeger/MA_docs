...
\vfill {\large
	...
	\LARGE \textbf{A Graphical Language for\\
	Specifying Security Models} \\ \vspace{2.5ex}
...
\vfill \parbox{\detailwidth}{
	...
	\vspace{2ex} \\
	\makebox[\detaildescriptionwidth][r]{Program:~~} Computer Science (Master) 2010\\  
	\makebox[\detaildescriptionwidth][r]{Student number.:~~} 46756  
	\vspace{2ex} \\
	\makebox[\detaildescriptionwidth][r]{Submitted:~~} Ilmenau, 29. November 2018
}
\vspace*{\fill}
\end{center}
\end{titlepage}
...
\setcounter{page}{1}
\phantomsection
\addcontentsline{toc}{chapter}{Contents} \markboth{Contents}{\textbf{Contents}}
\tableofcontents
...
Also usability tests are used to evaluate the language as well as the editor with a selected set of participants.
...
The actual implementation of the EL editor is subject of the subsequent \mbox{chapter 5}:
...
This will be the starting point in designing the graphical specification language in \mbox{chapter \ref{gsl}}.
...
Unfortunately \cite{Sandhu96} gives no details on how or why the notation is build up the way it is in his work.
...
An aspect oriented security model is defined as 
\begin{gather*}
...
With \textit{multiplicities} an element can be constrained regarding its number in context of a relationship.
The notation is placed at the connection point of class and relationship components.
...
This means that the user is free to choose on which particular part he wants to work.
...
A GUI has to be designed in a way it can be operated mostly intuitively.
...
These are positioned at the connection points of sets and relationships, similar to UML's multiplicities (see section \ref{UML_additional_annotations}) or ERD's capacities (see section \ref{ER_additional_annotations}).
...
Just like UML/ERD the mathematical expression is depicted directly as textual expression without any additional graphical elements or symbols.
The expression can be $2^X$ when directly used at a set $X$, 2\textsuperscript{$\wedge$} or $2^*$ otherwise, e.g.\ at an \textit{operator element} (see section \ref{operator_elements}) that involves multiple elements.
...
\label{operator_elements}
The second extension is the \textit{operator element}, which allows specifying an operator for operations with one or more operands.
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=\linewidth-4.5cm]{../figures/chapter3/relationships.png}
...
While mappings do not require roman numerals (due to having just one incoming and one outgoing arrow), relations do.
Relations that exclusively refer to identical sets such as $\mathit{RH} \subseteq R \times R$ constitute an exception to that rule and can omit roman numerals because the order is no necessary information for conversion (e.g.\ figure \ref{fig:HIX2}).
...
\begin{figure}[htb]
	...
	\caption{Set with its definition modeled externally}
...
The line element is only legally applicable between a set and an operation, set or relation element.
...
It may be necessary to add operator elements to build up a more complex term.
...
As in the previous step it may be necessary to add operator elements to build up a more complex term here, too.
...
On a more abstract level the graph is structured in nodes and edges, just like an mathematical graph.
...
For more examples and insights in how to construct those models see \mbox{chapter \ref{evaluation}} \textit{Evaluation}.
...
Arrows can be deleted, annotated and inverted in their direction.
...
\noindent
\texttt{QMainWindow} is the default window Qt creates after program start.
...
\noindent
After set creation we add the relations $\mathit{UA}$ and $\mathit{PA}$ by creating the corresponding shapes and positioning them between the participating sets.
...
Note that it is also possible to create a dedicated element for $2^R$ instead of the arrow annotation.
...
Generally this includes all operator elements (like $\times$ in figure \ref{fig:RBAC96_v5}), but may also include \textit{secondary sets} like $O$ and $\mathit{OP}$ in this model.
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=\linewidth-2.5cm]{../figures/chapter6/RBAC96_v5.png}
