...
For this the notation used by \cite{Amthor18} is investigated regarding its consistency and general weaknesses.
Afterwards possible adjustments to solve these problems and to improve the notation are proposed.
In section \ref{gsl_elements} the graphical elements the system uses are described. 
%, followed by section \ref{gsl_structure}, which depicts the overall structure of the system.
...
The major problems of the graphical EL notation used by \cite{Amthor18}, which makes it inconsistent, are the ambiguously depicted relationships between elements and the ambiguous method of classifying elements.
In the following we want to isolate and describe the different issues.

\begin{mdframed}[style=mystyle,frametitle=Note]
Henceforth all possible connections between elements like mappings, relations, constraints, etc.~are referred to as \textit{relationships}.
\end{mdframed}


\vspace{4mm}
\noindent
\textbf{Problem 1: Set expressions in relationships}
\vspace{1mm}
...
\vspace{4mm}
\noindent
\textbf{Problem 2: Multiple arguments in relationships}
\vspace{1mm}

\noindent
Relationships can have multiple input arguments.
Unfortunately \cite{Amthor18} does not depict any mapping example with multiple arguments.
It is unclear how to depict those.
...
\noindent
The first naive attempt to extend the given notation would be drawing a line from $S$ and $O$, merge in one point and ending with an arrow head in $R$ (figure \ref{fig:inconsistent_two_arguments}).
...
\vspace{4mm}
\noindent
\textbf{Problem 3: Order of operations in relationships}
\vspace{1mm}
...
\vspace{4mm}
\noindent
\textbf{Problem 4: Ambiguously depicted categorization}
\vspace{1mm}
...
Therefore we have to either extend the dashed rectangle for $LS_2$ up to category $C$ (which then inevitable overlaps with other undesired horizontal categories and therefore gets associated with them) or we have to depict $E$ two times at once (in $LS_2$ and $C$), which makes the figure inconsistent again, but also redundant and thus confusing.
...
This makes it universal to depict any mathematical expression needed in context of a set argument.
...
\vspace{2mm}
\noindent
\textbf{Solution to problem 2: Operation type indicators}
\vspace{1mm}

\noindent
The second extension is the \textit{operation type indicator}, which allows specifying the type of an operation between elements of a relationship.
...
\vspace{4mm}
\noindent
\textbf{Solution to problem 3: Categorization by coloring}
\vspace{1mm}
...
Abandoning the idea of horizontal aligning still leaves the option to use dashed rectangles to classify elements.
In the worst case this would lead to every item in the figure being encapsulated into its very own rectangle.
This adds multiple new elements to a figure and makes it unnecessarily complex to determine all elements of a category as a result.
...
The following figures are all based on these colors.
...
Mappings and relations can be extended with additional rhombus objects to indicate the type of operation two or more sets are connected with.

Unlike its origin notation they can now indicate not just its relationship type (mapping, relation, constraint), but also the operation type that describes how its arguments are connected, when there is more than one argument.
...
So, to enhance the visual perception, we continue using black arrows and lines to maximize the contrast in combination with a white background.
To still be able to classify relationships by color, relationship elements need a sufficient enclosed area that can represent its color.
Relations already have such area: The rhombus shape holding the operation type symbol.
Relations always have at least one operation type symbol.
Mappings do not.
...
\noindent
Handling two or more arguments is possible now as well as handling multiple operations and any order of operations as depicted in figure \ref{fig:relationship_operation_2args} and \ref{fig:relationship_operation_3args}.
When having multiple operation types like in figure \ref{fig:relationship_operation_3args} the name of the relationship is encapsulated in the last object.

\begin{figure}[htb]
	\centering
	\includegraphics[scale=0.4]{../figures/chapter3/relationship_operation_2args.png}
...
\begin{figure}[htb]
	\centering
	\includegraphics[scale=0.4]{../figures/chapter3/relationship_operation_3args.png}
...
In rare cases the order of arguments participating in \textit{one} operation is relevant for the operation itself (e.g.~$\setminus$ for set substraction).
To indicate and determine the order of arguments roman numerals can be used as annotations as depicted in figure \ref{fig:operation_order}.

\begin{figure}[htb]
	\centering
	\includegraphics[scale=0.4]{../figures/chapter3/operation_order.png}
...
\section{Example}
\label{gsl_example}
In this section we want to depict some example \cite{Amthor18} uses in our newly developed notation.
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=\linewidth]{../figures/chapter3/RBAC96_v1.png}
...
Text Text Text Text Text Text.
Text Text Text Text Text  Text Text Text Text Text Text Text Text Text Text Text Text Text Text Text Text Text.
Text Text Text Text Text Text Text TextText Text Text Text Text Text Text Text Text Text Text Text Text Text Text Text Text Text Text Text.% essential: find interfaces/break points where graphical and where mathematical expression is reasonable
