...
The operator elements from solution 2 also allow expressing power sets (with an appropriate symbol inside its shape e.g.\ $\euscr{P}$ or $2^{\Box}$).
...
While using annotations is more space-saving and compact, the operator element can act as its own entity and multiple other relationships can refer to it (without using annotation duplicates for every single relationship).
...
Examples for mapping and relation are depicted in figure \ref{fig:relationships}.
...
\noindent
As already stated power sets can be expressed using annotations and operator elements to express a operation between two other elements as seen in figure \ref{fig:relationship_annotation}.
