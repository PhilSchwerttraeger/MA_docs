...
\subsection{Sets}
Regarding their shape sets remain the same (circles/ellipses), but now have colored areas to indicate category memberships.
...
\begin{figure}[htb]
	...
	\caption{Set element with two differently categorized subsets}
	\label{fig:subsets}
\end{figure}
...
\begin{figure}[htb]
	...
	\caption{Example for a set element with its internal definition}
...
Unlike its origin notation they can now indicate not just its relationship type (mapping, relation, constraint), but also the operation type that describes how its arguments are connected.
Relationships have their name next to it.
In the case of mappings the name is encapsulated into a rectangle.
This is done to have an area to depict its color for categorization.
Many -- especially bright -- colors like yellow do not have a high contrast to their background (generally white), so coloring lines or object's outlines is hard to see.
To enhance the visual perception we decided to continue using black arrows and lines to maximize the contrast in combination with a white background.
To still be able to classify relationships by color, they need an element to represent its color.

\begin{figure}[htb]
	\centering
	\includegraphics[scale=0.4]{../figures/chapter3/relationships.png}
	\caption{Relationships: Mapping, relation and constraint}
	\label{fig:relationships}
\end{figure}


% Annotation: If 1 do not show
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=\linewidth]{../figures/chapter3/RBAC96.png}
