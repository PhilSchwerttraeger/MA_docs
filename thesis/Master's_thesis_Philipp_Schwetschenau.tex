...
\noindent
The categories list is initialized with an empty model with the EL categories for $m=1$ (see section \ref{EL} of the fundamentals chapter), but can be modified while runtime or by importing projects.
It also holds all EL meta information such like the corresponding XML tag identifiers and colors (not depicted in figure \ref{fig:class_diagram}).

For the internal organization all graphical elements in the scene have an \textit{unique element identifier} (\texttt{EID}) to address them.
The \texttt{EID} of an item is not allowed to change during its lifetime.
The allocation of \texttt{EID}s is implemented as incrementing integers.
