...
\noindent
To counter the ambiguous depiction of sets in context of a relationship we need a better ways of depicting set expressions like power sets.
For \textit{unary} set operations we introduce \textit{relationship annotations}.
...
\vspace{2mm}
\noindent
\textbf{Solution to problem 2: Operation indicator}
\vspace{1mm}
...
To avoid proximity and overlapping conflicts the operation indicator is encapsulated in a graphical shape: a rhombus (based on ERD relationships, see page \pageref{fig:ERD_relationship}).
