...
In section \ref{editor_structure} a general overview of the window structure is given and the work-flow concept is explained.
Section \ref{editor_sections} then gives insight into the several sections of the editor with their purposes und functions.
...
Figure \ref{fig:GUI_tab1_graphical} shows the main window of the editor with its separation into 4 sections.
The two sections at the top (tabs, import/export) will remain always visible, while the two sections underneath will dynamically change depending on the tab selected in the tabs section.

\begin{figure}[htb]
	\centering
	\includegraphics[width=\linewidth-5cm]{../figures/chapter4/GUI_layout.png}
	\caption{Editor: Sections}
...
As a consequence of this decision there is for example no main menu on top.
This leads to more space for the drawing area and therefore for the graphical EL model going to be created.

To achieve a fast work-flow functions should be case-dependent and accessible directly at the point of interest, in this case close to the graphical elements.
Common functions like creating sets are therefore accessible through the context menu (right click on mouse or touch-and-hold on touchscreen-based devices).
...
Relations and mappings can be created between two sets by drag and drop.
The same way additional lines and arrows can be added between existing elements (for example to create a mapping between 3 or more elements).

Moving the drawing area's view is done with holding down the right mouse button (touchscreen domain: two fingers touch-and-hold), zooming with the mouse wheel (touchscreen domain: pinch-in/out).
Alternatively the familiar key commands Ctrl and +/- allow zooming, when no mouse wheel is available.
...
The section at the top left-hand side is the tab section.
...
The concept and work-flow are already described in section \ref{GUI_concept}.

On the right-hand side of the window is the \textit{side section}.
After selecting an element on the drawing area this section displays the element's properties and values.
The selection of properties will change according to the type of element selected (e.g. relations will have a \textit{operation} property, while sets do not).

At the top right-hand side of the window is the \textit{general} section.
Two buttons allow importing and exporting a model.

When switching to the \textit{table} tab in the tab section the drawing area and side section will be replaced with a single section containing a table (see figure \ref{fig:GUI_tab2_table}).
...
It is based on the tabular form for results of EL-based model engineering \cite{Amthor18} uses (see figure \ref{fig:EL_table}).

\begin{figure}[htb]
	\centering
	\includegraphics[width=\linewidth]{../figures/chapter4/GUI_tab2_table.png}
	\caption{Editor: Table tab}
...
\begin{mdframed}[style=mystyle,frametitle=Note]
The following proposed function is treated as an optional feature..
\end{mdframed}
A gear button in the general section (next to import/export) opens the settings inside the side section (see figure \ref{fig:GUI_tab1_graphical_settings}).
The settings subsection \textit{View} allows toggling the visibility of several model properties (e.g. show/hide annotations).
...
This menu would overlap the depicted model in the drawing area and the menu has to be entered again and navigated through after every change.

The proposed settings section allows a browser-like fast configuration of the desired level of detail with toggle buttons.
The changes can be instantly observed in the depicted model.
...
This also supports satisfiscing (principle 3 of usability \citep{Tidwell11}), because the user can easily use trial-and-error to find the function he needs.

\begin{figure}[htb]
	\centering
	\includegraphics[width=\linewidth]{../figures/chapter4/GUI_tab1_graphical_settings.png}
	\caption{Editor: Settings}
	\label{fig:GUI_tab1_graphical_settings}
\end{figure}
