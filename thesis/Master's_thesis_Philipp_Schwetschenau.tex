...
\noindent
Next step is to connect $O$ and $\mathit{OP}$ to $P$.
...
\noindent
For this we have to iterate through all basic elements (sets, mapping, relations, constraints) and perform three steps for each element:
First we collect the identifier for each element: We obtain $\mathcal{M}_{\mathit{RBAC}}$.
To obtain the \textit{semantical application} of EL $\mathit{sem}$ we need to identify the colors and corresponding EL categories for each element with the help of the depicted color legend or a color legend reference.
Finally we construct the individual definition for each element by visiting all arrows/lines and connected elements recursively (\textit{graph search}): We obtain all element definitions (see definition \ref{definition:RBAC}).
...
\subsection{CoreABAC}
Besides RBAC we want to depict another, but different type of security model that is based on attributes: The \textit{Attribute-Based Access Control} (ABAC) model.
