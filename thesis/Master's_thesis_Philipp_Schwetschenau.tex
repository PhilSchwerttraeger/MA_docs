...
\label{gsl_design}
In this chapter we develop a system to graphically model the Entity Labeling aspect (see section \ref{EL}) of Aspect-oriented Security Engineering.
...
Due to being highly tailored to one specific domain established notations and visualization systems for structural and semantic properties like UML or ER (see section \ref{graphical_notations}) are impractical for direct reuse in other domains.
A new specialized domain like EL requires a newly designed graphical system.
In reality certain concepts and ideas from related systems are getting (partly) adopted and adapted accordingly, when they semantically fit.
...
With this said we want to start off with the graphical notation \cite{Amthor18} uses to visualize the EL aspect and try to analyze and expose its problems.



\subsection{Analyzing the EL notation used by Amthor}
The two major problems of the graphical EL notation used by \cite{Amthor18}, which makes it inconsistent and ambiguous, are the depicted relationships between elements and the manner of categorization.
...
One is that all mappings in the visual summary ignore the presence of power sets or similar expressions that extend sets, e.g.~$ua: U \rightarrow 2^R$ is drawn as a simple arrow from $U$ to $R$.
...
Unfortunately \cite{Amthor18} does not depict any mapping example with multiple arguments, but we can easily imagine a simple mapping with two arguments: e.g.~a mapping that takes a subject's and an object's identification to decide whether an action is allowed or not (IBAC, see section \ref{IBAC}):
\begin{align*}
S \times O \rightarrow R 
\end{align*}

\noindent
The ad-hoc idea is to draw an arrow from $S$ and $O$, merge in one point and ending with an arrow head in $R$ (figure \ref{fig:inconsistent_two_arguments}) is not sufficient in terms of consistency, because the type of operation is not specified ambiguously; the operator can vary (here: $\times$, but may be $\cup$ or $\cap$ just as well).
...
\noindent
Besides that, when handling three or more arguments the order of operations (may be mathematically indicated with brackets) has to be considered and depicted as well for the sake of consistency:
\begin{align*}
\lbrace U1 \cup U2 \rbrace \times O \rightarrow R
\end{align*}
...
Additionally elements can be categorized by surrounding them with a dotted box (e.g.~\textit{R} into $LS_2$ in figure \ref{fig:ambiguous_categorization}).
The horizontal alignment seems similar to a table categorization with columns, but unlike a table the graphical depiction uses arrows to connect elements to indicate a relationship.

\begin{figure}[htb]
	\centering
	\includegraphics[width=\linewidth-12cm]{../figures/chapter3/ambiguous_categorization.png}
	\caption{Ambiguous categorization of a relation}
	\label{fig:ambiguous_categorization}
\end{figure}

When connecting two elements, that are positioned in different sections, arrows inevitably run through all sections lying between them.
But arrows do not always connect elements across two sections.
It is possible that they connect elements that are further away from each other or belong to the same section.
For example \textit{RH} in figure \ref{fig:ambiguous_categorization} is incorrectly associated with $LS_1$).
So arrows will automatically be associated with
\begin{enumerate}[label=\alph*)]
\item (all) sections lying between elements they connect, when these elements are categorized into different categories or 
\item the category of a single element, when the relationship is a loop
\end{enumerate}
which is both not preventable and undesired in many cases.

But not just arrows, also sets have a similar issue, because they can be member of multiple categories at once.
In figure \ref{fig:ambiguous_categorization} this could lead to a situation where $LS_2$ gets another element $X$, which is also member of a category not horizontally adjacent to $LS_1$.
Therefore we have to either extend the dotted rectangle for $LS_2$ up to the other category (which inevitable overlaps with other undesired horizontal categories and therefore gets associated with them) or we have to depict $X$ two times at once (inside $LS_2$ and the other category), which makes the figure inconsistent and thus confusing and redundant.
...
\vspace{2mm}
\noindent
\textbf{Solution 1: Relationship annotations and type indicators}
\vspace{1mm}
...
These are simliar to UML's multiplicities (see section \ref{UML_additional_annotations}) or ER's capacities (see section \ref{ER_additional_annotations}) and are positioned at the connection points of sets and relationships.
A relationship annotation describes a specific form of an element in context of a relationship it is part of.
Just like UML/ER the mathematical expression (e.g.~$2^R$) is depicted directly as textual expression without any additional graphical elements or symbols.
This makes it universal to depict any mathematical expression.

At this level of detail specifying special graphical elements (e.g.~differently shaped arrow heads) for every possible expression is not reasonable due to the amount of different elements needed.
...
The second extension is the \textit{type indicator}, which allows specifying the type of operation inside a relationship.
In the notation \cite{Amthor18} uses, relationships do not specify any internal operation type, just the relationship type (mapping, relation, constraint).
...
Again, just like before, it is not reasonable to introduce new graphical elements for the purpose of indicating the type of operation at this level of detail and with that amount of different possible types.
...
To avoid proximity and overlapping conflicts in case of short arrows or intersecting arrows the textual indicator is placed inside a graphical shape: a rhombus (based on ERD relationship objects, see section \ref{ERD}).
This graphical element is the logical center and connection point for all arrows from and to elements participating in this relationship.



\vspace{4mm}
\noindent
\textbf{Solution 2: Categorization by coloring}
\vspace{1mm}

\noindent
To counter the second major problem of the graphical EL notation used by \cite{Amthor18} we need a better way to depict memberships to categories.
...
Expanding the method into two dimensions will not solve this issue either.
Therefore aligning elements to one (or two) axis is not suitable for the task of classification.

As described in section \ref{gestalt_laws} gestalt laws can help to solve graphical-based tasks.
...
Coloring can indicate an element's classification without proximity, aligning or altering its shape and is applicable to sets as well as to relationships.
Also multiple memberships can be depicted by multi-coloring the surface of sets or the lines of arrows.
