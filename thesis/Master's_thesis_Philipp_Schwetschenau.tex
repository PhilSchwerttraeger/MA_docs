...
%
%\noindent
Similar procedure follows for the assignments $user$ and $roles$.
But unlike a relation a assignment has input and output participants.
...
\noindent
Furthermore we can optionally alter the model to a custom desired level of detail.
We decide that the depiction of $P$'s sub elements is not necessary for us in the context we want to use the model.
We decide to compress the model, because several elements are not associated with any EL category and are therefore less important than the other elements.
...
This results in a more compact graphical EL model, limited to EL-associated elements only.
...
For this three persons with different degree of knowledge
