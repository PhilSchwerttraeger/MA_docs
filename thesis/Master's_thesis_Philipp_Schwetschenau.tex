...
Also the properties section right to the drawing area will change according to the current element selection in the drawing area.
...
It contains the drawing area underneath as well as the properties section to the right-hand side of the drawing area.
...
On the right-hand side of the window is the \textit{properties section}.
...
At the top right-hand side of the window is the import/export section.


When switching to the table tab in the tab section on the top left-hand side the drawing area and properties section will be replaces with a single section containing a table.
...
Optionally there could be an additional section for view settings to toggle the visibility of model properties (e.g. show/hide annotations).
This lets the user adapt the view to the complexity of the depicted model.

The classical approach would be a main menu on top with a view section containing several items to enable/disable (state indicated with a checkmark).
This menu would overlap the depicted model in the drawing area and the menu has to entered again and navigated through after every change.

The proposed view settings section allows a browser-like fast configuration of the desired level of detail with toggle buttons.
