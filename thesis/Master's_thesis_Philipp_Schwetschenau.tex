...
The second chapter discusses essential fundamentals, that are relevant for reading and understanding this work.
...
We will first start with the \textit{visual summary} used by \cite{Amthor18} and investigate it in order to reveal its problems.
After isolating its problems we will propose possible matching concepts to solve these and to construct a new graphical specification language regarding design goals.

After covering the graphical specification language chapter 4 then describes requirements and the overall structure of the EL editor, that is going to be developed in conjunction with this work.
Also the general control concept and optional advanced features are discussed in this chapter.

The actual implementation of the EL editor is covered in the following chapter 5:
Qt as implementation base is introduced and the implementation's structure will be described on a reasonable abstraction level.
After that event handling and the import/export mechanism of the editor are explained in more detail.
...
Chapter 7 discusses open issues with the language as well as the editor to give a short outlook for further improvement and possible extensions.

The final chapter summarizes this work and it results.
...
%\subsection{ABAC$_\alpha$}
%\label{eval_language_abacalpha}
