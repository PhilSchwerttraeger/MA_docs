...
\section{Language}
\label{future_GSL}
% mathematical elements just as defined: mappings relation sets
...
ABAC$_\alpha$ defines elements we can not depict with the current language means: Sets of (attribute) functions.
These sets contain functions, that map to target sets depending on the type of the attribute function itself.
When we try to depict this model graphically (see figure \ref{fig:ABACalpha}) the result is not isomorphic due to information loss.
A possible solution is to extend the language by adding required elements.

\begin{figure}[bht]
	\centering
	\includegraphics[width=\linewidth]{../figures/chapter7/ABACalpha2.png}
	\caption{ABAC$_\alpha$ in the graphical EL specification language}
	\label{fig:ABACalpha}
\end{figure}
...
Therefore the number of additional elements the language can be reasonably extended with is limited as well.
...
Due to already handling two categorization methods, specifying element types through their geometrical shapes and EL categories through colors, extending the language with additional categorization concepts is not recommended.
...
Currently the XML-based file format does not support the import and export of any categories other than the standard EL categories (for $m=1$), which is necessary for the additional feature of creating and modifying custom categories and category colors.
For the latter a dynamic widget has to be provided that allows adding, removing and modifying of categories. 
For every category its name, XML tag and color has to be configured.
A button for retrieving the standard EL preset can help configuring the  correct ELM format (Entity Labeling Markup) for compatibility with model analysis and verification tools after export.

Regarding the graphical elements themselves, the depiction of texts inside element shapes can be improved, e.g.\ with \textit{dynamic centering and resizing} depending on actual properties being set, not set or having very long values.
Furthermore formulas can be visually enhanced by integrating a dedicated Latex module for handling the correct depiction of mathematical terms.
