...
\begin{figure}[htb]
	\centering
	\includegraphics[width=\linewidth-3cm]{../figures/chapter3/HIX.png}
	\caption{HIX example with $m=2$ \citep{Amthor18}}
	\label{fig:HIX}
\end{figure}

The layout was adapted to better compare both figures, but is not limited to this form in comparison to its original notation.
By removing the need of spatial alignment and by introducing colors for categorizing, models now can be arranged freely regarding its graph layout.

More examples ($RBAC_3$, $ABAC$) can be found in the appendix on page \pageref{}.
