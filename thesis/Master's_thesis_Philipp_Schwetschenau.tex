...
Other than problem 2, power sets are exclusively applied to just one set.
...
\subsection{Solution 2: Operator Indications}
...
Arrows or lines can connect such operator elements.
...
In rare cases also the order of elements participating in a operation is relevant for the operation itself, e.g.\ for $\setminus$ (set difference).
...
The color white is therefore preallocated and not available for any EL category.
...
Also individual benefits of the compact form (formula) versus the external graphical depiction with elements from above are similar here.
...
To counter this problem we can create an \textit{additional rule} that assumes elements with undetermined EL-memberships (e.g.\ sets only mentioned in context of a formula) are always associated with no EL category.
...
The conditions for using a sets formula have been described in section \ref{solution_alternate_forms} before.
...
\label{relationships}
In the former \cite{Amthor18} notation simple arrows depict mappings, double-sided arrows depict relations and dashed-lined arrows depict constraints.
...
Relations already have such area with their dedicated element shape, mappings do not.
So for mappings we create an additional shape to obtain a sufficient coloring area.
...
So in contrast to the original notation we can now specify not just a relationship type (mapping, relation, constraint), but also how its sets interrelate internally (see figure \ref{fig:relationship_operation_2args}).
...
\begin{figure}[!htb]
	\centering
	\includegraphics[scale=0.15]{../figures/chapter3/relationship_operation_2args.png}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[scale=0.15]{../figures/chapter3/relationship_operation_3args.png}
	\caption{Mapping with multiple different operations $f:A \times ( B \cup C ) \rightarrow Z$ }
	\label{fig:relationship_operation_3args}
\end{figure}

\noindent
In rare cases the order of elements participating in a operation is relevant for the operation itself (e.g.~$\setminus$ for set substraction).
...
\begin{figure}[!htb]
	\centering
	\includegraphics[scale=0.15]{../figures/chapter3/operation_order.png}
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=\linewidth-7cm]{../figures/chapter3/relationship_annotation.png}
...
As proposed in \textit{Solution 5: Alternate Forms} (section \ref{solution_alternate_forms}) subsets can also be depicted externally.
...
\begin{figure}[htb]
	\centering
	\includegraphics[scale=0.15]{../figures/chapter3/subsets_externally.png}
...
For this the mathematical expression has to be rebuilt with rhombus shapes in the correct order of operations.
%\begin{figure}[htb]
%	\centering
...
\noindent
Optionally we can now alter the appearance of the model to a desired level of detail (with some downsides discussed later on).
...
\noindent
Important to mention here is that both models (figure \ref{fig:RBAC96_v5_variant} and \ref{fig:RBAC96_v5}) are not necessarily semantically equal without assuming the additional rule from .
...
We can counter this problem by assuming that elements with unclear EL-memberships (e.g.\ $O$ in textual form) are always associated with no EL category.
...
One major difficulty was the difference between relations and set modifiers.
While in the first design iteration of the language relations and set modifiers were depicted by the same graphical element (rhombus shape) and could only be distinguished by having an identifier or not, they are now clearly differentiated with their own shapes to avoid misunderstandings.
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=\linewidth]{../figures/chapter6/refinedElements.png}
