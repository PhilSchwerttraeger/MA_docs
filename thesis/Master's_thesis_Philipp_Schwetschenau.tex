...
This chapter does not aim at explaining the whole source code or all functions, but at giving an overview on the implementation's structure and at giving some detailed insights towards it's core functionality.

For this we will first take a look at the \textit{Qt} framework, the implementation base for this editor, in section \ref{implementation_base}.
After that the general structure of the implementation itself will be explained in section \ref{implementation_structure}, followed by more detailed information on the \textit{graphics scene}, its central graph-based widget for drawing elements and how it is extended to fit the requirements for this editor.
Also the concept for importing and exporting the data from and to an XML-based file format for persistent storage is depicted in section \ref{implementation_importexport}.
In section \ref{implementation_sections} the different GUI sections of the editor are shown and their corresponding functions are outlined.
...
One part of the Qt is its IDE \textit{Qt Creater}, which is used to develop the EL editor in this work.
The main advantage in using Qt Creator rather than any other development environment is it's integrated design mode, which allows quickly building a prototype layout graphically, which then can be used and refined in the code.

Furthermore Qt is a good choice, because it heavily focuses on GUI development and has powerful classes to implement graph-based editors with custom items like the EL editor.
...
The main structure is heavily based on Qt's class structure for the classes we used and their relationships and is depicted as UML class structure in figure \ref{fig:Qt_class_structure}.
...
In the actual implementation almost all classes are derived from corresponding Qt classes and are extended by several features.
One example for this is \texttt{GraphicalElement}, which inherits from \texttt{QGraphicsItem} and adds private variables for EL element properties such as EID.

\noindent
The \texttt{GraphicsScene} holds functions to add new elements to the scene (e.g.\ \texttt{addSet()}).
It also holds a list of all \textit{EIDs} and a list of all element \textit{categories}.
...
The \texttt{GraphicsScene} also holds all EL meta information such like the corresponding XML tag identifiers and colors (not depicted in figure \ref{fig:class_diagram}).
...
After deleting an element the corresponding \texttt{EID} is free and can be reallocated to new objects again.

\begin{mdframed}[style=mystyle,frametitle=Note]
The \texttt{EID} differs from the attribute \texttt{identifier}.
The latter is the textual identifier of the EL element, e.g.\ \textit{U} for a \textit{Users} set.
Unlike the \texttt{identifier} attribute the \texttt{EID} is not allowed to change after set once.
\end{mdframed}

\noindent
All four element types that can be added to the scene (\texttt{Set}, \texttt{mapping}, \texttt{relation} and \texttt{constraint}) inherit from \texttt{GraphicalElement}.
...
All \texttt{QString}-typed attributes are displayed with an classical text edit widget.
The description text edit widget is multi-lined.
The element's categories are depicted with a list of checkable entries.
After the horizontal line the element-exclusive attributes are listed (in case of \ref{fig:sidesection} the set's formula).
Any changes in the side section will be instantly propagated to its element's attributes with every keystroke.



\section{Events}
Cursor-based behavior like selecting and marking items or drawing lines and arrows with drag and drop is implemented with the help of Qt's \texttt{mousePressEvent}, \texttt{mouseMoveEvent} and \texttt{mouseReleaseEvent} functions.
These are implemented on several levels.
The top-most visible item (text items included) in the scene receives mouse events first.
After reacting to the mouse event, the item can pass the event on to its underlying items or discard it.
\texttt{contextMenuEvent}.

Calling the context menu follows the same concept.
Currently there are two layers the context menu is callable from: \texttt{GraphicalElement} when an element is clicked or \texttt{GraphicsScene} when free space of the scene is clicked.
Figure \ref{fig:contextMenu_scene} shows the context menu after clicking an item in the scene.
...
The XML scheme follows the \textit{Entity Labeling Markup} specification in \citep[appendix C.2, p.~196]{Amthor18}.
