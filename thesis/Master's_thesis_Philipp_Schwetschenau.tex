...
\vspace{4mm}
\noindent
\textbf{1) Ambiguously depicted relationships}
\vspace{1mm}
...
\vspace{4mm}
\noindent
\textbf{2) Ambiguously depicted categorization}
\vspace{1mm}
...
\noindent
To counter the first problem of the graphical EL notation used by \cite{Amthor18} we need a better way to depict relationships.
...
Unfortunately the occurrence of falsely associating elements described above is unavoidable with the concept of horizontal aligning.
Expanding the idea of these sections into two dimensions will not solve this issue either.
Aligning elements to one (or even two) axis is therefore not suitable for the task of classification.

As described in section \ref{gestalt_laws} gestalt laws can help to solve graphical-based tasks.
The manner to classify with horizontal alignment utilizes the \textit{law of proximity}, but the \textit{law of similarity} seems much more suitable for this instead, because elements might be widely distributed.
