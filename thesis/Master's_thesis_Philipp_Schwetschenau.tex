...
\noindent
The second extension is the \textit{operation indicator}, which allows specifying an operator for operations with one or more operands.
...
Solution to problem 2 also allows expressing a power set (with an appropriate symbol inside its shape e.g.\ $\euscr{P}$ or $2^{\Box}$).
...
Internal definitions of sets can be depicted as textual formulas inside their shapes, but can also be graphically modeled outside of them with the help of other elements.
...
Note that the formula can neither indicate the category memberships of an element nor allows participating sets to be referenced by any other element in the model.
Beyond that it is not even possible to determine the category a set is associated with, but also whether a set is categorized or not categorized at all.
To counter this problem we can create an \textit{additional rule} that assumes elements with unclear EL-memberships (e.g.\ sets only mentioned in context of a formula) are always associated with no EL category.
...
For them shortcuts can be provided that combine multiple operators into just one element to reduce the needed amount of elements and therefore the visual complexity.

We introduce such shortcut element for relations.
A relation over the sets $P_1, P_2, \dots, P_n$ is defined as subset of their cartesian product: 
$R \subset P_1 \times P_2 \times \dots \times P_n$.
