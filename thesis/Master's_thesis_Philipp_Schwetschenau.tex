...
\noindent
\texttt{QGraphicsView} is a viewport and implements all view-related function such as zooming or navigating.
...
The import and export functions \texttt{XMLtoScene} and \texttt{sceneToXML}are implemented inside \texttt{MainWindow}.
Due to not having a strict separation in view and model components (regarding the popular architectural pattern \textit{Model View Controller}) the element's model data and visual properties and functions are all located inside its instance itself.
To export them into a structured format we can not iterate through all items on the scene, because this would include also texts and edges.
But we can iterate through the \texttt{GraphicsScene}'s list of \texttt{EID}s to find valid elements and then get all relevant data of each item according to its element type.

For the file handling Qt's \texttt{QFile} and \texttt{QFileDialog} classes are used, for dealing with XML in particular the \texttt{QXmlStreamWriter} and \texttt{QXmlStreamReader} classes.
...
Graphical scene: navigation, mouse modes
Context Menu
Items: Rename, selected, move
