...
Subject of this chapter is the evaluation of both, the graphical specification language for EL developed in chapter \ref{gsl} as well as the EL editor prototype, which was designed in chapter \ref{editor_design} and described regarding its implementation in chapter \ref{implementation}.
...
Finally all results are summarized and evaluated altogether in section \ref{eval_results}.
...
The goal here is to show that the language is suitable to model and depict selected relevant EL models and its elements with an adequate and useful \textit{expressiveness}.

The second method for the evaluation of the language as well as method to evaluate the editor consists of \textit{usability tests} with selected users.
The results should give insight in the language's and editor's properties of supporting \textit{usability} and \textit{operability} and in how reasonable the suggested work flow is for both.
...
To assess the language regarding its expressiveness this chapter constructs and depicts use cases in form of security models in context of EL.
...
RBAC is a security model, where the access control is based on roles.
It was already described in detail in section \ref{RBAC} of the fundamentals chapter (see page \pageref{RBAC}).
...
For this we take its elements and translate them to their graphical counterparts.

RBAC consists of sets, relations and mappings.
First we create all basic sets $U, R, P, S$ with circles for each set.
...
Arrows have to point from sets or to sets accordingly.
...
\begin{figure}[htb]
	\centering
	\includegraphics[scale=0.13]{../figures/chapter6/RBAC96_v5_variant_stage3.png}
	\caption{Modeling RBAC (step 2 and 3): Create relations and mappings}
	\label{fig:RBAC96_v5_variant_stage3}
\end{figure}

\noindent
Now we have to connect $O$ and $OP$ to $P$.
For this we need to rebuild the mathematical expression with rhombus shapes in the correct order of operations.
...
After that we can color the elements background to the colors of the according categories.
Note that elements that are not relevant for EL can be left blank (\textit{uncategorized}).
This includes all operation elements like $2^{\Box}$, but also $O$ and $OP$ in this model.
The resulting final model is depicted in figure \ref{fig:RBAC96_v5_variant}.

\begin{figure}[htb]
	\centering
	\includegraphics[scale=0.13]{../figures/chapter6/RBAC96_v5_variant.png}
...
The element then has to be connect to all elements participating in that constraint with dotted-lined arrows.
...
sem(LS) &= \lbrace S, R, RH \rbrace\\
sem(LA) &= \lbrace user, roles \rbrace\\
...
We decide that the depiction of elements that define $P$ is not necessary for us, because those elements are not associated with any EL category and are therefore less important than the EL-associated elements.
...
%This section will depict the CoreABAC security model in the graphical specification language for EL.
%Due to being very similar to the procedure for RBAC in the previous section, we will just depict the final model, not every step to construct it.
%
%CoreABAC is defined as following:

%TODO: CoreABAC definition


\subsection{Usability Tests}
\label{eval_language_usability}
As stated above the language is also evaluated with the help of usability tests.
For this three probands with different degrees of knowledge regarding security models and EL where asked to use the language as well as the editor.
All of them where students with background in model-based security engineering currently writing their master's thesis.

For every proband there was a approximately two hour meeting.
Every meeting was divided into two parts.
First the EL aspect and the graphical specification language for EL proposed in this work were presented and discussed.
Afterwards the probands were asked to use the language to model the RBAC security model (see section \ref{RBAC} and \ref{eval_language_rbac}) graphically by themselves.
For this the formal definition of the model was given (\ref{definition:RBAC}).
The task was executed by hand drawing as well as with a software tool, the EL editor developed in this work, later on.
In this section we will focus on the language part of it.
For the software part see section \ref{eval_editor_usability}.
After those meetings there was a additional final meeting with all probands and supervisors to summarize and evaluate the results of the tests together and to isolate the main concerns and aspects to improve the language.

While in the first proposal of the language relations and operations were depicted by the same graphical element (rhombus shape) and could only be distinguished by having an identifier or not, they are now clearly differentiated with their own shapes to avoid misunderstandings.
...
Also relations and mappings were altered regarding their shapes to visually support their properties and usage in a more intuitive way (see figure \ref{fig:refinedElements}).

\begin{figure}[htb]
	\centering
	\includegraphics[scale=0.13]{../figures/chapter6/refinedElements.png}
...
\label{eval_editor}
The editor being one artifact of this work, also was evaluated regarding its usability with user tests.
Due to being just a preliminary prototype of the software editor designed in chapter \ref{editor_design} not all features were available by the time of usability tests taking place.



\subsection{Usability Tests}
\label{eval_editor_usability}
The evaluation of the editor with usability tests was done combined with the usability tests described in section \ref{eval_language_usability}.
The probands were given the task to model a given security model with the graphical EL specification language by hand but also with the help of the EL editor.
...
But other then the overall design the desired work flow of drawing arrows by default instead of moving objects was not as intuitive as assumed.
...
Besides the mentioned aspects above several improvements for the editor's work flow were suggested as well as behavioural bugs of the editor were revealed and documented, that might act as pool for further development in the future.
...
Designed with the goal of being unambiguous and to support the EL process visually in the first place the graphical specification language developed in this work could be further improved in a second iteration regarding its usability and intuitive usage with the help of the results of proband tests.
For this several elements were modified in such way they now have more intuitive shapes, which are also better to distinguish.

Beside the language also the EL editor prototype, which was developed as tool to construct such EL models with software, was evaluated through user tests.
The results revealed i.a. some work flow weaknesses.
